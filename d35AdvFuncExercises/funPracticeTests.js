...
describe("map filter reduce", function() {
    ...
    it("tests map with squares", function() {
        function squares(number) { return number * number; }
        assert.deepEqual(myMap([1, 2, 3], squares), [1, 4, 9]);
    });
    it("tests map with cube", function() {
        function cube(number) { return number * number * number; }
        assert.deepEqual(myMap([1, 2, 3], cube), [1, 8, 27]);
    });
    it("tests map with decimel", function() {
        function decimal(number) { return number * 10; }
        assert.deepEqual(myMap([1, 2, 3], decimal), [10, 20, 30]);
    });
    ...
    it("tests filter with equal to 2", function() {
        function equal(number) { return (number === 2); }
        assert.deepEqual(myFilter([1, 2, 3], equal), [2]);
    });
    it("tests filter with odd", function() {
        function odd(number) { return (number % 2 !== 0); }
        assert.deepEqual(myFilter([1, 2, 3], odd), [1, 3]);
    });
    ...
    it("tests filter with < 3", function() {
        function less3(number) { return (number < 3); }
        assert.deepEqual(myFilter([1, 2, 3], less3), [1, 2]);
    });
