...
describe("filter practice", function() {
    ...
    it("includes even age", function() {
        assert.strictEqual(peopleArray.includes(6));
    });
});
describe("reduce practice", function() {
    ...
    it("reduce all sum of numbers", function() {
        assert.deepEqual(sumOfNumbers(numArray), 422);
        assert.deepEqual(numArray, [5, 0, 7, 77, -20, 300, 51, 2]);
    });
    it("reduce average of numbers", function() {
        assert.deepEqual(averageOfNumbers(numArray), 52.75);
        assert.deepEqual(numArray, [5, 0, 7, 77, -20, 300, 51, 2]);
    });
    it("reduce max of numbers", function() {
        assert.deepEqual(maxOfNumbers(numArray), 300);
        assert.deepEqual(numArray, [5, 0, 7, 77, -20, 300, 51, 2]);
    });
    it("reduce max of ages", function() {
        assert.deepEqual(maxOfAges(peopleArray), { name: "Barney", age: 80 });
        assert.deepEqual(peopleArray, [{ name: "Sam", age: 15 }, { name: "William", age: 6 }, { name: "Lucy", age: 13 }, { name: "Barney", age: 80 }]);
    });
});
describe("use chain practice", function() {
    beforeEach(function() {
        peopleArray = [{ name: "Sam", age: 15 }, { name: "William", age: 6 }, { name: "Lucy", age: 13 }, { name: "Barney", age: 80 }];
    });
    it("chain average age of people with even ages", function() {
        assert.deepEqual(averageOfEvenAges(peopleArray), 43);
        assert.deepEqual(peopleArray, [{ name: "Sam", age: 15 }, { name: "William", age: 6 }, { name: "Lucy", age: 13 }, { name: "Barney", age: 80 }]);
    });
    it("chain average age of people with odd ages", function() {
        assert.deepEqual(averageOfOddAges(peopleArray), 14);
        assert.deepEqual(peopleArray, [{ name: "Sam", age: 15 }, { name: "William", age: 6 }, { name: "Lucy", age: 13 }, { name: "Barney", age: 80 }]);
    });
});